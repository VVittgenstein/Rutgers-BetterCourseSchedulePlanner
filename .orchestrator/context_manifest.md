...
- [task-007] Applied the approved Stage A non-product cleanup in merge `f5031a1`: added `.orchestrator/stage-a/07-cleanup-application.md`, untracked seven local/runtime artifacts while preserving them as ignored local files, added ignore coverage for `scripts/poller_checkpoint.json` and `data/refresh_queue.json`, moved legacy AI workflow evidence into `docs/archive/stage-a-legacy/`, left root `read_only.md` as a forwarding pointer, and moved `notebooks/incremental_trial.md` to `reports/incremental_trial.md`. Release pack disposition, optional `.env.example`, `frontend/src/dev`, and `reports/field_validation_details.mdpart` remain deferred.
- [task-008] Added `.orchestrator/stage-p/01-public-divergence-and-exposure-policy.md`. It confirms raw `dev` -> `main` merge is unsafe: `main` has real Auto Refresh/Scheduled Fetch product files missing from `dev`, while `dev` has internal `.orchestrator/`, `AGENTS.md`, and `docs/archive/stage-a-legacy/` artifacts that must not be public. Public candidate construction must use sanitized replay, preserve `main`'s product feature, exclude internal/runtime/private/history surfaces, merge `.gitignore` intent from both branches, and keep `far/` as temporary local evidence only.
