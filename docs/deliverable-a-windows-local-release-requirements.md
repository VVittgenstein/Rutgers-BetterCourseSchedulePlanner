...
> **P1 JOINT REVIEW IN PROGRESS - ROUND 1**

Status: behavior-level recovery/revalidation complete; joint review open; not approved for P2
Language: English
Deliverable: A, the Windows-local Rutgers BetterCourseSchedulePlanner release
Decision gate: renewed joint P1 Review; P2 remains blocked
Historical NGAT candidate provenance: dev merge `a4b035a586a4b14fc3a75698caf99badce869fd5`
Superseded review record: [`p1-a-recovery/08-mainline-review.md`](p1-a-recovery/08-mainline-review.md)
Reopen decision: [`p1-a-recovery/09-p1-reopen-legacy-capability-gap.md`](p1-a-recovery/09-p1-reopen-legacy-capability-gap.md)
Legacy capability inventory: [`p1-a-recovery/10-legacy-capability-inventory.md`](p1-a-recovery/10-legacy-capability-inventory.md)
Prior validation snapshot: [`p1-a-recovery/11-single-line-p1-validation.md`](p1-a-recovery/11-single-line-p1-validation.md)
Current P1/P2/delegation clarification: [`p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md`](p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md)
Current revalidation: [`p1-a-recovery/13-single-line-p1-revalidation.md`](p1-a-recovery/13-single-line-p1-revalidation.md)
Active review record: [`p1-a-recovery/14-joint-p1-review.md`](p1-a-recovery/14-joint-p1-review.md)

## 1. Status, scope, and authority

This document remediates the previously accepted partial P1 baseline produced
from the independently audited NGAT candidate and corrected in the former
dual-line workflow. Its current decisions remain intact, while the companion
legacy inventory now enumerates the old project's product capabilities and
behavior semantics from requirements, code, tests, architecture, task records,
Git/GitHub history, release artifacts, and the main-machine recovery corpus.
P1 exists because the abandoned task-015 line and long project pause left the
User without reliable memory of the old product; P1 must merge that recovered
historical layer with every later accepted decision, including Rust/shared
architecture, A/B delivery, watch/audio changes, and mandatory UI phases. It
does not authorize P2-P7 work. (Evidence: `ML-E061`, `ML-E062`, `ML-E067`,
`ML-E076`, `ML-E080`; remediation/clarification records:
`09-p1-reopen-legacy-capability-gap.md` and
`10-legacy-capability-inventory.md`,
`12-p1-p2-purpose-and-delegation-clarification.md`.)

The words **must** and **shall** below identify accepted P1 target requirements supported
...
| Layer | Treatment in this accepted P1 target | Resolving evidence |
|---|---|---|
...
| Later accepted 0A-0C decisions | Operative target constraints for A as accepted by the current-line P1 Review. | `ML-E024`, `ML-E025`, `ML-E026`, `ML-E027`, `ML-E029`, `ML-E030`, `ML-E043`, `ML-E044`, `ML-E047`, `ML-E048` |
| Current process clarification | P1 is explicitly the provenance-preserving merge of old project memory and new accepted decisions; P2 is the dependency-aware all-and-only adjudication; Codex-supervised subagents/child threads are permitted without creating another authority line. | `p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md` |
| Behavior-level legacy inventory | Every source-backed old capability, removed behavior, future/non-goal, and drift point is registered for explicit P2 adjudication. Presence is not automatic v1 membership. | `LEG-A-001`-`LEG-A-159` in `p1-a-recovery/10-legacy-capability-inventory.md` |
...
evidence-ID family used by this target to those resolving ledgers.
...
| `HIST-A-008` | P1 was created because the abandoned task-015-to-025 local-release line and a long pause left the User unable to reliably remember the original feature/design intent. Persistent local history and Git/GitHub history must reconstruct that memory before modern adjudication. | Current direct User clarification corroborated by immutable `0a61028` Phase 1 planning and the July 10 transcript. The unmerged task-015 matrix is discovery evidence, not accepted truth. | `p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md` |

## 3. Later accepted 0A-0C constraints

These decisions post-date the original history and control the current
target where they resolve an older ambiguity or supersede a stale
...
| `DEC-A-004` | A/B v1 exclude email, SMTP, SendGrid, mail configuration, and mail-worker delivery. No push, app, or invented system-notification product is substituted; active-WebUI browser audio is the current alert channel. Email reminders remain future work and shall be tracked on GitHub without exposing any v1 mail surface. | `ML-E008`, `ML-E009`, `ML-E010`, `ML-E025`, `ML-E056`, `TH-E007` |
...
| `DEC-A-009` | P1 reconstructs old product memory and merges it with later accepted decisions. P2 assigns `KEEP`, `RECOVER`, `REPAIR/REDESIGN`, `REMOVE`, `DEFER`, or `USER DECISION REQUIRED` to every recovered and extant surface, including dependency-aware removal boundaries. P3-P6 design and reconcile A/B, and approved P7 implements, validates, packages, and conditionally publishes. No phase may cross its review gate. | `ML-E058`, `ML-E061`, `ML-E062`, `ML-E067`, `ML-E068`, `ML-E073`, `ML-E076`; `p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md` |
| `DEC-A-010` | NGAT is retired. The current Codex may dispatch bounded subagents or child threads, but they remain subordinate: they cannot make unresolved product decisions, approve phases, or cross review gates, and Codex must inspect/integrate their work before joint review here. | `docs/dual-delivery-workflow.md`; `p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md` |
...
| Step | Accepted ordinary-user outcome | Evidence |
|---:|---|---|
...
| 4 | The local core acquires and refreshes Rutgers course-directory data, validates staged data before replacement, and presents a usable last-known catalog rather than a half-refreshed catalog. The last-known-catalog preservation wording is accepted as bounded P1 synthesis from the atomic-replacement rule; exact retention and retry mechanics remain later design and validation work. | `ML-E046`, `ML-E050` |
| 5 | The user can apply simultaneous course/section/meeting filters, including multi-value and strict day/time-window behavior, inspect course and section information, and see section status without knowing or choosing an API route. The exact all-and-only filter and view set remains the P2 decision recorded through the legacy inventory. | `ML-E026`, `RC-E011`, `RC-E012`, `TH-E002`, `LEG-A-044`-`LEG-A-105` |
...
## 5. Accepted P1 target requirements

### 5.1 Product purpose and user boundary

| Requirement | Accepted P1 target requirement | Evidence |
|---|---|---|
...
| `A-PROD-004` | This P1 target shall state supported outcomes without deciding the P2 all-and-only membership of unresolved historical surfaces. | `ML-E062`, `ML-E068`, `PH-E032`, `TH-E002`, `TH-E003` |
...
| Requirement | Accepted P1 target requirement | Evidence |
|---|---|---|
| `A-UI-001` | The ordinary-user WebUI shall support behaviorally specified course search and filtering, course inspection, section inspection, and current section status display. P1 no longer treats the phrase "search and filtering" as sufficient: `LEG-A-044`-`LEG-A-083` enumerate retained, removed, combined, and stubbed filter behavior for P2. | `ML-E026`, `RC-E011`, `RC-E012`, `LEG-A-044`-`LEG-A-083` |
...
| `A-UI-004` | P7.2 shall explicitly use `$industrial-brutalist-ui` and `$design-taste-frontend` while implementing the shared UI, including loading, empty, error, and interactive states. P7.3 shall begin only after P7.2 and shall explicitly use `$emil-design-eng` to independently polish the integrated UI's interaction, motion, responsiveness, input, reduced-motion, accessibility, and performance behavior. The P7.3 review artifact shall use `Before \| After \| Why` for each proposed refinement. | `ML-E022`, `ML-E070`, `ML-E071`, `ML-E074` |
...
| `A-CFG-002` | A shall keep application data, configuration, and logs in an appropriate per-user Windows location. The accepted P1 evidence does not fix the exact location or mechanics; the eventual implementation shall define that location and any upgrade/migration behavior explicitly. | `ML-E047` |
| `A-CFG-003` | A shall not expose email, SMTP, SendGrid, mail-worker, or mail-configuration settings in v1. The removed email-reminder capability shall be tracked on GitHub as future work, not represented as current configuration or fallback behavior. | `ML-E008`, `ML-E009`, `ML-E025`, `ML-E056`, `TH-E007` |
...
marked **candidate inference** retain that evidence classification after P1
acceptance; their unresolved mechanics require confirmation at the named later
design or validation gate.

| Requirement | Accepted P1 target requirement | Treatment | Evidence |
|---|---|---|---|
...
| `A-VAL-006` | Every P7 task shall produce its own clean, substantive commit and push that commit to the approved remote branch after its task-level completion gate. Commit quality, public cleanliness, reviewability, and safety outrank contribution-graph appearance. P7.2 UI implementation and P7.3 polish shall remain separate sequential tasks, completion records, and commit series. | `ML-E074`, `ML-E077` |
...
| `A-VAL-008` | P7 shall build and audit a new Windows package against the accepted target. **Release-candidate provenance/validation inference:** historical release archives, names, tags, or deleted GitHub assets shall not be reused as proof that the new release candidate is ready. | `ML-E072`, `ML-E075`, `PH-E016`, `PH-E018` |
| `A-VAL-009` | P1, P6, and every required P7 subphase shall stop at their named joint-review gates in this one authoritative conversation; final product/release acceptance remains with the User and Codex review mode. | `ML-E066`, `ML-E067`, `ML-E068`, `ML-E076`; `docs/dual-delivery-workflow.md` |

### 5.9 Recovered legacy capability surface incorporated into P1

The full 159-row inventory in
[`p1-a-recovery/10-legacy-capability-inventory.md`](p1-a-recovery/10-legacy-capability-inventory.md)
is a normative P1 input to P2, not optional background reading. The groups
below make the recovered product surface visible in this target without
pretending that P1 has already made P2's keep/remove/redesign/defer decisions.

| Candidate group | Behavior-level content recovered for P2 | P1 treatment | Inventory rows |
|---|---|---|---|
| `A-LEG-BOUND-001` | Public Rutgers SOC course browsing plus section-opening monitoring for ordinary users. | Strong historical purpose, compatible with current A. | `LEG-A-001`-`LEG-A-003` |
| `A-LEG-BOUND-002` | No account/NetID, no automatic WebReg enrollment, no degree audit/graduation planning/complex prerequisite validation, and public SOC as authority. | Explicit historical boundaries; P2 must preserve or explicitly reopen them. | `LEG-A-004`-`LEG-A-007` |
| `A-LEG-BOUND-003` | Multi-term, multi-campus, level scope, bilingual UI, mobile/desktop use, open distribution, and privacy. | Historical candidates; exact supported scope and validation remain P2/P6 work. | `LEG-A-008`-`LEG-A-014` |
| `A-LEG-BOUND-004` | Old email/Discord channels versus current browser-audio-only A/B parity. | Current direct decisions supersede the old channels; history remains visible. | `LEG-A-015`-`LEG-A-016` |
| `A-LEG-DATA-001` | Complete selected-scope course and section catalog, including course, section, meeting, instructor, cross-list, prerequisite/Core/exam/delivery/location fields when Rutgers supplies them. | Strong historical requirement candidate; field-by-field completeness must be adjudicated and tested. | `LEG-A-017`-`LEG-A-024` |
| `A-LEG-DATA-002` | SQLite, migrations, full/incremental loads, staging/atomic replacement, multiple targets, retry/rate discipline, hashes, and checkpoint recovery. | Mixed current constraints and historical implementation candidates; P2/P3 decide mechanics. | `LEG-A-025`-`LEG-A-033` |
| `A-LEG-DATA-003` | Manual fetch/status/log UI, dynamic and fallback dictionaries, exam-dictionary mismatch, FTS, pagination/sorting, ten-minute cadence, and orphan scheduled/auto refresh. | Implemented, drifting, and orphaned states remain separate; package/source presence is not proof of a working feature. | `LEG-A-034`-`LEG-A-043` |
| `A-LEG-FILTER-001` | Keyword, term, campus, campus-location, subject, level, credits, Core, exam, prerequisite, delivery, open-only, day, and time-window filters. | Retained current filter candidates, now enumerated instead of summarized. | `LEG-A-044`-`LEG-A-058` |
| `A-LEG-FILTER-002` | AND across fields, OR within multi-value fields, coherent matching-section semantics, and simultaneous combinations such as day + delivery + time. | Implemented/tested semantics that P2 must explicitly retain or redesign. | `LEG-A-059`-`LEG-A-062` |
| `A-LEG-FILTER-003` | Active chips, individual/group/all clearing, validation, debounce, cancellation, caching, URL state, pagination, and sorting. | Current interaction/query behavior plus an orphan URL integration; P2/UI review pending. | `LEG-A-063`-`LEG-A-072` |
| `A-LEG-FILTER-004` | Removed course-number, Index/section-number, instructor, explicit status/waitlist, meeting-campus, building/room, permission, tags/keywords, school, and cross-list filters. | Historical requirements/implementations must each receive a P2 decision rather than disappearing because the rewrite removed them. | `LEG-A-073`-`LEG-A-082` |
| `A-LEG-FILTER-005` | Advanced direct section-query schema whose implementation always returns empty results. | Known false/stub surface; recover or remove/demote in P2. | `LEG-A-083` |
| `A-LEG-VIEW-001` | Course list, section previews, complete registration/schedule detail, loading, error/retry/trace, empty, and pagination states. | Current list is functional but sparser than the old FR-01 field promise. | `LEG-A-084`-`LEG-A-090` |
| `A-LEG-VIEW-002` | Weekly/calendar view, meeting blocks, shared List/Calendar state, and section drawer/watch action. | Strong historical candidates; schedule component is implemented only in the development playground. | `LEG-A-091`-`LEG-A-094` |
| `A-LEG-VIEW-003` | Saved presets, share links, compact view, conflict detection, ICS export, and free-time search. | Mixed historical candidates and explicitly later ideas; each remains P2 pending. | `LEG-A-095`-`LEG-A-100` |
| `A-LEG-VIEW-004` | Virtualization, responsive filter/result layout, reduced motion, accessible interaction, and complete language-state behavior. | Mixed drift, design candidates, and implemented i18n; final UI belongs to P7.2/P7.3 after P2. | `LEG-A-101`-`LEG-A-105` |
| `A-LEG-WATCH-001` | Old Index-centric subscribe/manage flow, durable subscription records, idempotency, remembered contacts/devices, active-subscription discovery, transition events, dedupe, and checkpointing. | Historical user outcomes and mechanics are separated; current live-session model supersedes the durable delivery architecture. | `LEG-A-106`-`LEG-A-114` |
| `A-LEG-WATCH-002` | Old outage handling, local claim polling, fixed WebAudio/toasts, email, and Discord. | Failure outcome may remain useful; claim/mail/Discord mechanics are superseded or excluded. | `LEG-A-115`-`LEG-A-120` |
| `A-LEG-WATCH-003` | Explicit live start, nine-watch cap, connection memory, WebSocket, every fresh Open, no debounce, volume, one-shot/continuous, browser limitations, latest-value delivery, adaptive polling, latency aspiration, and unresolved section key. | Current accepted watch/audio constraints; exact protocol and identity remain later design decisions. | `LEG-A-121`-`LEG-A-133` |
| `A-LEG-OPS-001` | Health/readiness, trace IDs, query metrics, and loopback binding. | Existing outcomes to adjudicate/port into the Rust implementation. | `LEG-A-134`-`LEG-A-138` |
| `A-LEG-OPS-002` | Node prerequisite, dependency installation, native rebuild, and C++ Build Tools requirement versus final no-runtime BAT package. | Concrete startup drift and regression cases, not acceptable release behavior. | `LEG-A-139`-`LEG-A-144` |
| `A-LEG-OPS-003` | Divergent archives, orphan packaged code, clean-machine and artifact validation, historical performance/cost/usability/coverage/deployment targets, truthful docs, platform chronology, notification boundaries, and shared-Rust architecture. | P2/P3/P6/P7 inputs; old metrics are not current claims until reaccepted and measured. | `LEG-A-145`-`LEG-A-159` |
...
| `OBS-A-006` | The broad task-015 matrix was unmerged and never passed provenance review. | It remains a discovery aid only and cannot supply feature membership or validate this target. | `PH-E014`, `PH-E015`, `RC-E018` |

## 7. Explicit P1 synthesis

These inferences connect accepted outcomes where the evidence does not provide
a fully specified requirement. P1 Review accepts their inclusion as visible,
bounded synthesis; they do not silently fix mechanics assigned to later gates.

| Inference | Accepted P1 synthesis | Supporting evidence |
|---|---|---|
...
| Conflict or correction | Evidence sides | Accepted P1 treatment |
|---|---|---|
...
| Historical email surfaces versus current v1 | Historical docs/UI contain email and disagree on behavior (`PH-E029`, `PH-E030`, `RC-E016`, `RC-E022`); accepted 0A/0C removes mail (`ML-E025`, `ML-E056`, `TH-E007`). | Current A/B v1 has browser audio and no email/mail configuration. Historical mail remains provenance context only, and email reminders are tracked on GitHub as future work. |
...
| P7 deployment scope | A side-branch expanded P7 into Vultr hardening/deployment; the User corrected the workflow so P7 ends with packages and this conversation later owns guided production deployment (`ML-E083`). | Production Vultr deployment is outside this A target and outside P7 packaging. |
| Workflow ownership/status | Older evidence captured a draft dual-line/NGAT workflow (`ML-E084`). The User later retired dual-line ownership while preserving A/B deliverables and P1-P7. | `docs/dual-delivery-workflow.md` now controls: Codex executes and User + Codex review in this one conversation. Codex-supervised subagents/child threads may perform bounded work without becoming another authority line. The old NGAT status is chronology only. |
...
|---|---|---|
| `EX-A-001` | Email reminders, SMTP, SendGrid, mail workers, mail configuration, and email fallback are excluded from v1. Email reminders remain GitHub-tracked future work. | `ML-E008`, `ML-E009`, `ML-E025`, `ML-E056`, `TH-E007` |
...
|---|---|---|
| Calendar, Compact view, saved views, share links, and presets | Behavior-level history is now recovered, but membership is not promoted by P1. Leave each item for explicit P2/UI review. | `PH-E027`, `PH-E032`, `RC-E026`, `TH-E003`, `LEG-A-091`-`LEG-A-100` |
...
| `UNR-A-009` | What is the explicit `KEEP`/`RECOVER`/`REPAIR-or-REDESIGN`/`REMOVE`/`DEFER`/`USER-DECISION-REQUIRED` disposition for every inventory row and every extant local/remote/release surface, including the dependency-safe removal boundary for each non-member? | P2 product-surface audit | `PH-E032`, `PH-E033`, `TH-E003`, `ML-E062`, `LEG-A-001`-`LEG-A-159`; `p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md` |
...
| Evidence prefix | Resolving reviewed companion | Locator-bearing section | Use in this accepted P1 target |
|---|---|---|---|
...
| `LEG-A-001`-`LEG-A-159`, resolved through `LC-E001`-`LC-E034` | [`p1-a-recovery/10-legacy-capability-inventory.md`](p1-a-recovery/10-legacy-capability-inventory.md) | Sections 3-5; each capability row names evidence IDs and each `LC-E*` entry has file/line, symbol, hash, or immutable Git locators | Complete behavior-level old-product inventory, implementation/removal chronology, current superseding decisions, stubs, orphans, and P2-pending membership |
| Current scope clarification | [`p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md`](p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md) | Sections 1-7; includes transcript lines and immutable commit/blob anchors | Task-015 origin, old-plus-new P1 merge, subordinate-agent allowance, and dependency-aware P2 closure invariants |
...
| P1 memory-reconstruction origin, old-plus-new merge, delegation, and P2 all-and-only closure | `p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md` |
...
| Exact multi-value, combined, day/time, retained, and removed filter behavior | `LEG-A-044`-`LEG-A-083`; `LC-E007`, `LC-E010`-`LC-E014` |
| Course/section field completeness, schedule views, presets/share, and later schedule ideas | `LEG-A-017`-`LEG-A-024`, `LEG-A-084`-`LEG-A-105` |
...
## 12. Renewed P1 review gate statement

This is a **remediated P1 candidate awaiting joint review**, not an approved
P2 input. It preserves the accepted current decisions and the three corrections
recorded in
[`p1-a-recovery/08-mainline-review.md`](p1-a-recovery/08-mainline-review.md),
while adding the complete behavior-level legacy inventory required by
[`p1-a-recovery/09-p1-reopen-legacy-capability-gap.md`](p1-a-recovery/09-p1-reopen-legacy-capability-gap.md).
The earlier verification snapshot is recorded in
[`p1-a-recovery/11-single-line-p1-validation.md`](p1-a-recovery/11-single-line-p1-validation.md).
The User then clarified P1's task-015/memory-recovery origin, the mandatory
old-plus-new merge, permitted Codex-supervised delegation, and P2's deeper
all-and-only standard in
[`p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md`](p1-a-recovery/12-p1-p2-purpose-and-delegation-clarification.md).
The corrected candidate is revalidated in
[`p1-a-recovery/13-single-line-p1-revalidation.md`](p1-a-recovery/13-single-line-p1-revalidation.md).

P2 has not started. Approval requires the User and Codex in this conversation
to inspect the remediation and either correct it or explicitly accept it as the
P2 input. No statement in this document authorizes P2 or implementation before
that gate. (Evidence: `ML-E058`, `ML-E061`, `ML-E067`, `ML-E076`, `ML-E089`,
`LEG-A-001`-`LEG-A-159`.)
