...
| Conflicts and unresolved items | Kept visible for the named later gate; no endpoint, identifier, UI, performance, or package-layout choice is invented here. | `ML-E028`, `ML-E032`, `ML-E081`, `ML-E082`, `ML-E083`, `ML-E090`, `TH-E002`, `TH-E003` |
...
| `DEC-A-005` | Watches are explicit active-WebUI sessions with at most nine section keys. Remembered selections are not active watches, and active state ends with the connection, tab/browser close, disconnect, or suspension. | `ML-E012`, `ML-E020`, `ML-E027`, `ML-E052`, `TH-E005` |
...
| 8 | Closing or suspending the active WebUI connection ends or interrupts the active watch. Audio while the browser/device is locked, closed, disconnected, or suspended is not guaranteed. | `ML-E016`, `ML-E027`, `ML-E052` |
...
| `A-PKG-005` | Product data, user configuration, and logs shall live under an appropriate per-user Windows location rather than requiring writes beside the unpacked application or into the source tree. The precise Windows directory and migration policy remain later design choices. | `ML-E047`, `ML-E090` |
...
| `A-PKG-007` | The accepted approximate four-item layout (`bcsp-local.exe`, `start.bat`, user documentation, and license) is direction only; later packaging design and clean-machine validation shall select and audit the final filenames and layout. | `ML-E048`, `ML-E072`, `ML-E090` |
...
| `A-WATCH-002` | Active watch state shall belong to the live connection and be released when the tab/browser closes, the connection ends, or the browser is suspended. Locally remembered choices may aid selection but shall remain inactive until the user explicitly starts watching. | `ML-E027`, `ML-E052` |
...
| `A-CFG-002` | A shall keep application data, configuration, and logs in an appropriate per-user Windows location. The eventual implementation shall define that location and any upgrade/migration behavior explicitly. | `ML-E047`, `ML-E090` |
...
| `A-ERR-005` | A disconnect or suspension shall end the active watch state. The UI should make inactive, disconnected, stale, and recovering states distinguishable rather than imply that monitoring remains live. Exact reconnect policy remains unresolved. | Lifecycle is accepted; visible-state wording is **candidate inference** from honest UI/error-state requirements. | `ML-E027`, `ML-E052`, `ML-E054`, `ML-E070`, `ML-E090` |
| `A-ERR-006` | Local diagnostics shall support health and freshness investigation and shall be written under the per-user log location without credentials. Exact format, retention, redaction, and support bundle behavior remain later design choices. | Supported outcome with unresolved mechanics. | `ML-E038`, `ML-E046`, `ML-E047`, `ML-E078`, `ML-E090` |
...
| `A-VAL-008` | Historical release archives, names, tags, or deleted GitHub assets shall not be reused as proof that the new candidate is release-ready; the final package must be freshly built and validated against the accepted target. | `PH-E016`, `PH-E018`, `RC-E018` |
...
| `INF-A-002` | Atomic staging implies that a failed refresh should leave the last successful catalog usable and retry later, but exact retention, rollback, and retry policy must be designed and validated later. | `ML-E050`, `PH-E031` |
...
| `UNR-A-007` | What exact crate/module split, checked frontend-contract mechanism, and internal ownership boundaries implement the accepted modular monolith? | P3-P5 design | `ML-E045`, `ML-E046`, `ML-E090` |
| `UNR-A-008` | What are the final archive name, executable name, package tree, per-user Windows directory, upgrade/migration policy, launcher timeout, and diagnostic/log format? | P3 design, P6 review, and P7 packaging validation | `ML-E047`, `ML-E048`, `ML-E090` |
