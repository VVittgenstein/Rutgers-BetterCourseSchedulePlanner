...
| Network | Outbound HTTPS to `classes.rutgers.edu` (data fetch/poller) and SendGrid/Discord if sending real notifications. |
| Credentials | `SENDGRID_API_KEY` (or SMTP) for mail, `DISCORD_BOT_TOKEN` (+ optional `DISCORD_PUBLIC_KEY`) for Discord. |
...
- Copy examples and adjust paths/IDs:
  ...
  - `cp configs/discord_bot.example.json configs/discord_bot.local.json` → fill `applicationId/defaultChannelId`, and ensure `botTokenEnv` matches the env var you will export.
...
4) **Start core services (use separate terminals)**
   - API server:
     ...
     ```
     Expected log: `Listening on http://0.0.0.0:3333`.
   - Frontend (runs Vite with proxy to API):
     ```bash
     cd frontend
     VITE_API_PROXY_TARGET=http://localhost:3333 npm run dev -- --host 0.0.0.0 --port 5174
     ```
   ...
   - Discord dispatcher:
     ```bash
     DISCORD_BOT_TOKEN=... tsx workers/discord_dispatcher.ts \
       --sqlite data/local.db \
       --bot-config configs/discord_bot.local.json \
       --allow-channel <channel_id> \
       --app-base-url http://localhost:5174 --idle-delay 2000
     ```
     Expected: send logs with traceIds; Discord channel/DM receives messages when notifications exist.
...
### C) Subscribe + receive mail/Discord
...
2. Create a subscription (email example):
   ...
   ```
   For Discord DM, change `contactType` to `discord_user` and provide the snowflake in `contactValue`.
...
4. Run dispatchers (`mail_dispatcher` and/or `discord_dispatcher` as above) until the `pending` count drops to 0. Check your inbox or Discord channel for the message; logs should show `status=sent` (mail) or `sent` result (Discord).
...
If you want a dry-run without live sends, use `npx tsx scripts/mail_e2e_sim.ts` (no provider traffic) and `npm run discord:test-send -- --dryRun --channel <id>`.
...
| Discord sends skipped | Channel not allowlisted or dry-run enabled | Pass `--allow-channel <id>`; set `testHooks.dryRun=false` and ensure `DISCORD_BOT_TOKEN` is valid; use `npm run discord:test-send -- --channel <id>` to verify credentials. |
