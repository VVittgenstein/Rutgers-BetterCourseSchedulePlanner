...
| FLT-S04b | 同步方式 | 只聚合课节的线上/远程组成部分，再判断 `同步 / 异步 / 同步异步混合` 之一是否在集合中；纯线下不适用并通过 | 线上组成部分的同步性或适用性无法确定 |
| FLT-S05 | 教师 | 教师名集合匹配（空白折叠后精确；取值来自动态字典） | 教师字段缺失 |
| FLT-S06 | 可上课时间 | 见 4.1 | 时间待定/缺失/非法；课节无时段记录且不是异步 |
| FLT-S07 | 上课地点 | 所有「需要到场」的实体时段都必须在所选集合内（跳过线上/远程与显式可选时段）；纯在线不适用并通过 | 无时段记录，或无法确定某个时段是否需要到场 |
...
### 4.2 FLT-S04b 同步方式的适用范围

同步方式是对**线上组成部分**的限制，不是对所有上课时段的第二次分类：

- 明确的线下时段不参与；课节没有线上组成部分时，本条件不适用并直接通过；
- 在线组成部分全为固定时段时是 `SYNC`，全为自行安排时是 `ASYNC`，两者都有时是 `MIXED`；
- 混合课必须只用其线上组成部分聚合。例如“线下固定 + 在线异步”匹配 `ASYNC`，不能因为整个课节同时含固定和异步时段而错误匹配 `MIXED`；
- 已声明在线/混合但找不到可证明的线上组成部分，或线上组成部分的同步性未知时，为 `UNCERTAIN`；
- 是否允许纯线下、纯在线或混合课由 FLT-S04a 授课方式决定；FLT-S04b 不替代授课方式。
...
若课节完全没有需要到场的实体时段，适用集合为空，地点条件按空约束通过；不得再把线上地点
`O`/`T` 拿回来与实体子校区比较。是否保留或排除纯在线课由“授课方式”筛选决定。因此同时选择
“在线”和 College Avenue 时，纯在线课仍可出现；只想看 College Avenue 实体课时，应取消“在线”。

以 2026-09-03 的 `92026 / NB` 完整真实快照复验：只启用 College Avenue 地点条件时，
得到 1,553 门课程 / 3,034 个课节。该总数包含地点条件不适用的纯在线课，也保留
「线下只在 College Avenue、另有线上时段」的混合课；旧的 1,514 课节结果来自把纯在线
课错误拿去与实体子校区比较的实现，不能继续作为现行语义基线。
