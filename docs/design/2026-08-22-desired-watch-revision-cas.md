...
> §2.7 Full Reset、§3 CAS 的检查**顺序**、§5.5 的**暂态/永久分类本身**、
...
> 验收条目、§10、§12 中依赖帧流的门。
>
> **2026-08-24 追加作废**（M0-M1-001 实现时按用户最终裁定执行）。上面
> 那份"仍然有效"清单在这两点上是错的，**恢复上下文时不得据旧清单**：
>
> - **§3.2 第 4 步里的目录准入整段作废。** CAS 不查 target / term /
>   section / gate；写入期唯一的准入是 9 门 post-state 上限。那些条件
>   全部改到**物化阶段**判定，因为它们每一个都是可撤销的：把一次
>   "暂时不行"冻结进 receipt 就成了永久答案。
> - **§5.6「永久失败：系统 CAS」整节作废**，§5.5 里"永久 → §5.6 系统
>   CAS 移出 desired + 通知"一行同样作废。**服务端永不代用户撤销意图**：
>   永久失败保留 desired、暴露原因、停止重试，STOP 由用户决定。
>   `retire_desired_watch` 及其 outcome 已从代码中删除。
>
> 当前合同以 `2026-08-23-desired-watch-reduced-scope.md`（v2）为准。
