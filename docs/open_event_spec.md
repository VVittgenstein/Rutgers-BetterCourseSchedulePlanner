...
| `dedupe_key` | TEXT | `sha1(term|campus|index|status_after|bucket_5m)`; prevents duplicate sends within the same freshness window. |
...
- **Default loop**: Each active `{term, campus}` is polled every **45–75 seconds** (45 s target + ±30% jitter). A round-robin queue prevents a single campus from starving others.
