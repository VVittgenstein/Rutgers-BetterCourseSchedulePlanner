...
2) Leave the window open. On first run it will install dependencies, migrate/create the SQLite file, and run a full data fetch for the default term/campus configured in `configs/fetch_pipeline.local.json`.
3) Your browser opens to `http://localhost:5174`. Use the web UI to pick the campus and term you want (e.g., NB campus + the target semester).
4) To stop, press Ctrl+C in the launcher window or close it.

## Pick a different term/campus for the data
- Defaults come from `configs/fetch_pipeline.local.json` (first entry in `targets[]`). Campus codes are the Rutgers SOC short codes (e.g., `NB`).
- Quick override without editing JSON: set environment variables before launching (`CSP_TERM=12026` and `CSP_CAMPUSES=NB` would target term 12026 for NB).
