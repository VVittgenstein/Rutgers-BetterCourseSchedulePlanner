...
bcsp_install_release() {
  ...
  if [[ -e "$target" ]]; then
    ...
    for entry in BUILD-PROVENANCE.json FRONTEND-CAPABILITIES.json LICENSE \
      MANIFEST.json SBOM.cdx.json SHA256SUMS THIRD-PARTY-NOTICES.txt VERSION \
      systemd caddy config ops docs; do
