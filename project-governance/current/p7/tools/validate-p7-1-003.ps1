...
function Invoke-Git([string[]]$Arguments,[bool]$AllowFailure=$false){
    ...
    try{$ErrorActionPreference='Continue';$output=@(& git -C $repo -c core.autocrlf=false @Arguments 2>$null);$code=$LASTEXITCODE}finally{$ErrorActionPreference=$old}
...
function Test-DeniedTaskPath([string]$Path){
    return $Path-match '(^|/)\.secrets(/|$)' -or $Path-match '(^|/)chat-log-[^/]*\.md$' -or $Path-match '^docs/(chat-log-|sessions/|p1-a-recovery/)' -or $Path-match '^project-governance/current/p1/' -or $Path-match '(^|/)(node_modules|dist|target|\.cache|\.ngagent|\.orchestrator)(/|$)' -or $Path-match '(?i)(\.sqlite3?|\.db|-wal|-shm)$'
}
...
try{
    ...
    if($null-ne$script:Contract -and [bool]$script:Contract.commitBoundary.finalized){
        $builder=@(& $builderPath -VerifyOnly 2>&1);Assert-True($LASTEXITCODE -eq 0)"evidence builder VerifyOnly failed: $((@($builder)|Select-Object -Last 8)-join ' | ')"
    }
